...
// 不加锁版本
void one_block_step_no_lock(Matrix& U, Matrix& B, Matrix& V, int l, int r) {
...
void* worker_thread(void* arg) {
    while (true) {
        ...
        if (g_pool.next_task_idx < g_pool.task_count) {
            ...
            // 执行任务
            one_block_step_no_lock(*(g_pool.U), *(g_pool.B), *(g_pool.V), task.l, task.r);
...
// 并行处理函数（线程池 + 动态任务队列）
void process_blocks_parallel(Matrix& U, Matrix& B, Matrix& V, std::vector<Block>& blocks) {
    ...
    while (true) {
        ...
        // 执行任务
        if (task.r > task.l) {
            one_block_step_no_lock(U, B, V, task.l, task.r);
        }
